...
from tree import Leaf, FixedTree, ListTree
...
def test_simplest():
    root = Leaf(single_type, 'value')
    assert render_text(root) == 'value'

def test_recursion():
    tree = ListTree(single_type, [
                     Leaf(single_type, 'value1'),
                     FixedTree(double_type, [
                                ListTree(single_type, []),
                                Leaf(single_type, 'value2'),
                               ]),
                     ListTree(single_type,
                              [Leaf(single_type, 'value3')]),
                    ])
...
def test_single_link():
    tree = Leaf(single_type, 'value')
    assert render(tree, 'link') == '<a href="link">value</a>'

def test_nested_links():
    tree = ListTree(single_type, [Leaf(single_type, 'value')])
...
def test_really_nested_links():
    tree = ListTree(single_type, [
                     ...
                     ListTree(single_type, [Leaf(single_type, 'value3')]),
                    ])
